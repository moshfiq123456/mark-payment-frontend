...
const Food = () => {
  ...
  const fetch = useCallback(()=>{
    api.get(`food/${token._id}?date=${params.date}&filterType=${params.filterType}&page=${Number(params.page)}&size=${Number(params.size)}`).then((res)=>{
  ...
  const handleDelete = (e) =>{
    api.delete(`food/${e._id}`).then((res)=>{
      console.log(res)
      fetch()
    }).catch((error)=>{
      console.log(error)
  ...
  const columns = [
    ...
    { 
      ...
      render: (row) => (
        <span className={` ${row.status ? 'bg-black' : ' bg-red-300'}`}>
