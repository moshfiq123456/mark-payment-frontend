...
const Icecream = () => {
  ...
  const fetch = useCallback(()=>{
    api.get(`payment/65dae4a6d24875baf64e6c1e?date=${params.date}&filterType=${params.filterType}&page=${Number(params.page)}&size=${Number(params.size)}`).then((res)=>{
  ...
  const handleSubmit = (value)=>{
    api.post("payment",value).then((res)=>{
      console.log(res)
  ...
  const handleDelete = (e) =>{
    api.delete(`payment/${e._id}`).then((res)=>{
      console.log(res)
      fetch()
    }).catch((error)=>{
      console.log(error)
